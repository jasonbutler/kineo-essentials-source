...
define(function (require) {
    ...
    var AdaptModel = Backbone.Model.extend({
        ...
        fetchAllDescendantIds: function() {
           
            if(this._children == undefined) return []; // or return undefined?

            var children = this.getChildren().models;
            var allDescendantIds = [];

            function addToList(children) {
                _.each(children, function(child){
                    allDescendantIds.push(child.get('_id'));
                    if(child.get('_children') && child.get('_children').length > 0) addToList(child.getChildren().models);
                });
            }
        ...
        getChildren: function () {
            ...
            console.log("getChildren: " + this._children);
